...
function StarGrade(props: StarGradeProps) {
    ...
    const filledStars = Array.from({ length: currentGrade }, () => <Star state="filled" />);
    const defaultStars = Array.from({ length: maxGrade - currentGrade }, () => <Star state="default" />);
        ...
        <div className={styles.starGrades}>
            {filledStars.length !== 0 &&
                filledStars.map((filledStar, index) => (
                    <div data-index={JSON.stringify(index)} key={index} onMouseOver={handleMouseOver}>
                        {filledStar}
                    </div>
                ))}
